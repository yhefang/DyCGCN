...
class Processor(IO):
    ...
    def seed_everything(self):
        torch.manual_seed(self.arg.seed)  # Current CPU
        torch.cuda.manual_seed(self.arg.seed)  # Current GPU
        np.random.seed(self.arg.seed)  # Numpy module
        random.seed(self.arg.seed)  # Python random module
        torch.backends.cudnn.benchmark = False  # Close optimization
        torch.backends.cudnn.deterministic = True  # Close optimization
        torch.cuda.manual_seed_all(self.arg.seed)  # All GPU (Optional)
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.train_feeder_args, model=self.arg.model),
            ...
            # self.data_loader['train'] = torch.utils.data.DataLoader(
            #     torch.utils.data.Subset(dataset=Feeder(**self.arg.train_feeder_args), torch.randperm(len(dataset))),
            #     batch_size=self.arg.batch_size,
            #     shuffle=False,
            #     num_workers=self.arg.num_worker * torchlight.ngpu(
            #         self.arg.device),
        ...
        if self.arg.test_feeder_args:
            self.data_loader['test'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.test_feeder_args, model=self.arg.model),
    ...
    def test(self):
        ...
        self.epoch_info['MSE'] = 0
        self.show_epoch_info()

    def start(self):
        self.io.print_log('Parameters:\n{}\n'.format(str(vars(self.arg))))
        self.best_performance = {'mean_loss':99999, 'RMSE':99999,'R2':0, 'EIR-1':0, 'EIR-5':0, 'EIR-10':0}
        # training phase
        if self.arg.phase == 'train':
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--device', type=int, default=[0,1,2,3], nargs='+', help='the indexes of GPUs for training or testing')
